...
class UpstoxOAuth:
    ...
    Upstox documents the v2 authorization-code endpoint. The client secret and
    authorization code are supplied only for the outbound request and are never
    stored in this object or included in errors.
    """
    ...
    async def exchange_code(
        ...
        *,
        ...
        self._require(code, "authorization code")
        self._require(client_id, "client id")
        self._require(client_secret, "client secret")
        self._require(redirect_uri, "redirect uri")
        data = {
            "code": code,
            "client_id": client_id,
        ...
        payload = await self._post(self.token_url, data=data)
        token = self._token_from(payload, "access_token")
        return BrokerToken(token, self._next_upstox_expiry())
...
class DhanOAuth:
    ...
    async def generate_consent(self, *, client_id: str, api_key: str, api_secret: str) -> str:
        self._require(client_id, "client id")
        self._require(api_key, "api key")
        self._require(api_secret, "api secret")
    ...
    async def consume_consent(self, *, token_id: str, api_key: str, api_secret: str) -> BrokerToken:
        self._require(token_id, "token id")
        self._require(api_key, "api key")
        self._require(api_secret, "api secret")
        payload = await self._request(
            "POST",
        ...
        token = self._token_from(payload, "accessToken")
        expiry = self._parse_expiry(payload.get("expiryTime"))
        return BrokerToken(token, expiry)

    async def renew(self, *, client_id: str, access_token: str) -> BrokerToken:
        self._require(client_id, "client id")
        self._require(access_token, "access token")
        payload = await self._request(
            "POST",
            self.renew_url,
        ...
        token = self._token_from(payload, "accessToken")
        return BrokerToken(token, self._parse_expiry(payload.get("expiryTime")))

    async def _request(self, method: str, url: str, *, params: dict[str, str] | None = None, headers: dict[str, str] | None = None) -> dict[str, Any]:
        try:
            async with httpx.AsyncClient(timeout=self.timeout, transport=self.transport) as client:
                response = await client.request(method, url, params=params, headers=headers or {}, timeout=self.timeout)
        except httpx.HTTPError as exc:
            raise BrokerAuthError(f"broker authentication transport failure: {type(exc).__name__}") from exc
        if response.is_error:
            raise BrokerAuthError(f"broker authentication returned HTTP {response.status_code}")
