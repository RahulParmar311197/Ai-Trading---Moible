...
class DhanBroker:
    ...
    async def reconcile_order(self, client_order_id: str) -> BrokerReconciliation:
        ...
        if not isinstance(data, dict) or not data:
            return BrokerReconciliation(client_order_id=client_order_id, matched=False, reason="order not found by correlation id")
        broker_order = self._map_order(data)
        return BrokerReconciliation(client_order_id=client_order_id, broker_status=broker_order.status, matched=True, broker_order=broker_order)

    @classmethod
    def _map_status(cls, raw_status: str) -> BrokerOrderStatus:
        status_map = {"TRADED": BrokerOrderStatus.FILLED, "PART_TRADED": BrokerOrderStatus.PARTIALLY_FILLED, "PENDING": BrokerOrderStatus.NEW, "TRANSIT": BrokerOrderStatus.OPEN, "CANCELLED": BrokerOrderStatus.CANCELLED, "EXPIRED": BrokerOrderStatus.CANCELLED, "REJECTED": BrokerOrderStatus.REJECTED}
        try:
            return status_map[raw_status.upper()]
