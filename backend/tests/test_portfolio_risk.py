...
from app.execution.portfolio_risk import (
    PortfolioPosition,
    PortfolioRiskError,
    PortfolioRiskLimits,
    assess_portfolio,
...
def limits(**overrides):
    values = {
        ...
        "max_single_position_notional": Decimal("100000"),
        "max_pair_correlation": Decimal("0.90"),
    }
...
def position(symbol, quantity, price, returns=()):
    return PortfolioPosition(
        ...
        returns=tuple(Decimal(str(value)) for value in returns),
    )


def test_assessment_aggregates_gross_net_and_pnl():
    result = assess_portfolio(
        [position("LONG", 10, 100), position("SHORT", -4, 50)],
        limits(),
    )
...
def test_gross_and_net_limits_are_independent():
    result = assess_portfolio(
        [position("A", 100, 100), position("B", -100, 90)],
        limits(max_gross_exposure=Decimal("10000"), max_net_exposure=Decimal("2000")),
    )
...
def test_high_positive_correlation_is_reported_and_rejects():
    result = assess_portfolio(
        [
            position("A", 1, 100, [1, 2, 3, 4]),
            position("B", 1, 100, [2, 4, 6, 8]),
        ],
        limits(max_pair_correlation=Decimal("0.90")),
    )
    assert not result.approved
...
def test_undefined_correlation_history_is_rejected():
    with pytest.raises(PortfolioRiskError, match="correlation is undefined"):
        assess_portfolio(
            [
                position("A", 1, 10, [1, 1, 1]),
                position("B", 1, 10, [1, 2, 3]),
            ],
            limits(),
        )
