...
def _response(status_code, *, json_data, url="https://ai.example.test"):
    return httpx.Response(status_code, json=json_data, request=httpx.Request("POST", url))
...
def test_http_provider_extracts_generic_output(monkeypatch):
    ...
    def fake_post(endpoint, *, json, headers, timeout):
        ...
        return _response(200, json_data={"output": {"summary": "ok"}}, url=endpoint)
...
def test_http_provider_extracts_openai_style_json_content(monkeypatch):
    def fake_post(endpoint, **kwargs):
        return _response(200, json_data={"choices": [{"message": {"content": '{"summary":"ok"}'}}]}, url=endpoint)

    monkeypatch.setattr(httpx, "post", fake_post)
    assert HttpAIProvider("https://ai.example.test").complete(system="system", payload={}) == {"summary": "ok"}


def test_http_provider_rejects_non_json_model_content(monkeypatch):
    def fake_post(endpoint, **kwargs):
        return _response(200, json_data={"choices": [{"message": {"content": "not json"}}]}, url=endpoint)

    monkeypatch.setattr(httpx, "post", fake_post)
    with pytest.raises(AIProviderError, match="non-JSON"):
        HttpAIProvider("https://ai.example.test").complete(system="system", payload={})
...
def test_http_provider_does_not_require_api_key(monkeypatch):
    ...
    def fake_post(endpoint, *, json, headers, timeout):
        ...
        return _response(200, json_data={"summary": "ok"}, url=endpoint)
